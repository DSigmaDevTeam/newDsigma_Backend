...
module.exports = async (req, res, next) => {
    ...
    jwt.verify(token, JWT_SECRET, async(err, payload) => {
        try {
            ...
            const permission = await Permission.findOne({where:{role_id:employee.roleId, module_id:2}});
            ...
            console.error(error);
            return res.status(500).json({success: false, message:`employeePermisson Error: ${error.message}`});
        }
    })
}